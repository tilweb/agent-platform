...
KI-Workplace is a multi-user AI workplace with a React+Vite frontend and Bun+Hono backend. It features multi-provider LLM orchestration, an extensible tool ecosystem, RAG-based knowledge base, OAuth integrations, task queues, and image generation.
...
High-volume directories use **date-based sharding** (`YYYY/MM` subdirectories): `tasks/`, `chats/`, `conversations/`, `generated-images/`, `exports/`. Task results (JSON) are stored alongside task YAMLs in the same bucket. Files are bucketed by ID timestamp; reads fall back to flat paths for backward compatibility. A one-time migration (`migrateSharding.ts`) runs at startup to move legacy flat files into buckets.
