...
| 18  | Lange Session-Dauer (7 Tage)          | ✅ Erledigt | **Sliding Sessions** implementiert: (1) Session-Timeout 3 Tage _Inaktivität_ (`auth/types.ts`), (2) Session wird bei jeder Auth-Anfrage verlängert (`middleware.ts`), (3) Max. absolute Lebensdauer 30 Tage (auch bei Aktivität). Verlängerung nur wenn >1h seit letzter Verlängerung (Performance).               | 2026-02-13 |
