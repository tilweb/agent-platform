...
├── hooks/           23+ Custom Hooks fuer Business-Logik
...
├── chats/              Chat-Verlaeufe (pro User, Sharding nach Datum)
...
├── knowledge-base/     Wissensbasisis-Sammlungen
├── connections/        Connector-Plugins + verschluesselte Tokens
...
POST /api/chat
  ...
  -> Agent-Loop:
     ...
     3. Tool-Calls erkennen und ausfuehren
     4. Tool-Ergebnisse zurueck an LLM
     5. Iteration bis fertig (max 5 / max 15 fuer Supervisor)
...
Supervisor-Agent
  ...
  -> Tool-Ausfuehrung im delegierten Kontext
...
| `/api/knowledge` | knowledge | Wissensbasisis |
