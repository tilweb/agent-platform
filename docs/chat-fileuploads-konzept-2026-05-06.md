...
└────────────────────────┘                                                       │
                                ...
                                ▼                                         ▼
                    ┌───────────────────────────────────────┐  ┌─────────────────────────────┐
                    │ ../data/chat-uploads/{sid}/{aid}/     │  │ Postgres                    │
                    │   ├─ original.<ext>                   │  │ messages.metadata.attachments│
                    │   ├─ content.md   (nur document)      │  │  (JSONB, nur ID + Filename + │
                    │   ├─ transcription.txt (nur audio)    │  │   Transcription/Preview)     │
                    │   └─ metadata.json   (Index)          │  │  → services/chatStorage.ts   │
                    └───────────────────────────────────────┘  └─────────────────────────────┘
                                ▲                                 ▲
                                │                                 │
   ┌────────────────────────────┴─────────────┐      ┌────────────┴───────────────┐
...
**Zwei Persistenz-Schichten**:
- **Disk**: Datei-Bytes + Markdown/Transkript/Manifest unter `data/chat-uploads/{sid}/{aid}/`
- **Postgres**: Chat-Verlauf mit Attachment-Metadaten (ID, Filename, Transkript, Preview) im `messages.metadata`-JSONB-Feld
...
## 6. Persistenz im Chat-Verlauf — Postgres

`backend/src/services/chatStorage.ts` ist die DB-Schicht, `backend/src/services/memory.ts` die Session-/Pending-Logik.

### 6.1 Tabellen
Schema in `backend/src/db/schema/chat.ts`:
- `chats` — ein Eintrag pro Chat-Session (Owner, Titel, Project-Ref, Timestamps)
- `messages` — eine Zeile pro Message; `metadata` als **JSONB** mit `agentId`, `routedBy`, `attachments[]`
- `chat_folders` + `chat_folder_members` — Folder-Organisation

**`messages.metadata` Beispiel-Inhalt**:
```json
{
  "agentId": "supervisor",
  "routedBy": "auto",
  "attachments": [
    {
      "id": "att-86d3b6d1-...",
      "type": "audio",
      "filename": "recording-1772345.webm",
      "mimeType": "audio/webm",
      "url": "/api/chats/chat-abc/attachments/att-86d3b6d1-.../stream",
      "transcription": "Heute ist Mittwoch.",
      "preview": null
    }
  ]
}
...
`MessageAttachment` ist exportiert in `chatStorage.ts:20–28`. **Kein Datei-Inhalt im JSONB** — nur Metadaten + Transkript + Vorschau. Die Bytes liegen weiter auf Disk; das `url`-Feld zeigt auf die `routes/attachments.ts`-Endpoints.

### 6.2 Pending-Attachments-Pattern
`memory.ts` hält eine **In-Memory-Map** `pendingAttachments` mit Key `userId::sessionId` (Z. 27 ff.). Der Upload-Pfad in `chat.ts` ruft `setPendingAttachments()`, der Message-Builder konsumiert sie via `popPendingAttachments()` und schreibt sie als Teil der `messages.metadata` in die DB.

> **Folge**: Wenn ein User Files hochlädt, dann den Browser schließt **bevor** er die Message abschickt, gehen die Attachment-Metadaten verloren — die Datei-Bytes liegen aber noch auf Disk. Manuelles Cleanup via `cleanupSessionAttachments()` oder die User-spezifische TTL-Logik (siehe Stolpersteine).

### 6.3 Was passiert beim Chat-Löschen?
`deleteChat()` in `chatStorage.ts:233` entfernt die Postgres-Zeilen (cascading auf `messages`). **Die Attachment-Dateien auf Disk bleiben liegen**. Wer das aufräumen will, muss separat `cleanupSessionAttachments(sessionId)` rufen — derzeit gibt es keinen automatischen Hook.
