...
export function useProjektmanagement() {
  ...
  const getPortfolioCosts = useCallback(async (portfolioId) => {
    const response = await apiGet(`/apps/projektmanagement/portfolios/${portfolioId}/costs`);
    if (response.status === 404) return null;
    if (!response.ok) throw new Error('Failed to get portfolio costs');
    const data = await response.json();
    return data.costs;
  }, []);

  const getPortfolioRisks = useCallback(async (portfolioId) => {
    const response = await apiGet(`/apps/projektmanagement/portfolios/${portfolioId}/risks`);
    if (response.status === 404) return null;
    if (!response.ok) throw new Error('Failed to get portfolio risks');
    const data = await response.json();
    return data.risks;
  }, []);
