...
export function useSearch() {
  ...
  const search = useCallback(async (searchQuery, connectedSourceIds) => {
    if (!searchQuery || searchQuery.length < 2) {
      setResults({ chats: [], knowledge: [], confluence: [], gdrive: [], gmail: [], pipedrive: [], contracts: [] });
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      setResults(data.results || { chats: [], knowledge: [], confluence: [], gdrive: [], gmail: [], pipedrive: [], contracts: [] });
