...
describe("generateExcel", () => {
  describe("basic output", () => {
    ...
    test("should produce a valid XLSX file (starts with PK magic bytes)", async () => {
      // XLSX is a ZIP-based format, so the first two bytes are always 'PK' (0x50 0x4B)
      const result = await generateExcel(minimalDoc);
  ...
  describe("section types", () => {
    test("should handle a text section", async () => {
      const doc: DocumentData = {
        title: "Text Only",
        metadata: {},
        sections: [{ title: "Notes", type: "text", content: "Some text here" }],
      };
      const result = await generateExcel(doc);
      expect(result).toBeInstanceOf(Buffer);
      expect(result.length).toBeGreaterThan(0);
    });
  ...
  describe("edge cases", () => {
    ...
    test("should handle a table with a Gesamt total row", async () => {
      const doc: DocumentData = {
        title: "Budget",
        metadata: {},
        sections: [
