...
import { authMiddleware, adminMiddleware, getCurrentUserId, getCurrentUser } from '../auth';
import {
  connectionRegistry,
  saveConnection,
  loadConnection,
 ...
 */
connectionRoutes.get('/admin/providers', authMiddleware, adminMiddleware, async (c) => {
  try {
    const providers = await connectionRegistry.getProviderInfos(); // ohne userId → kein per-User-Status
 ...
 */
connectionRoutes.put('/admin/providers/:id/enabled', authMiddleware, adminMiddleware, async (c) => {
